...
    
    // If sorting by pricePerQuantity, do not apply initial DB sort
    const dbSort = sortBy === 'pricePerQuantity' ? {} : sort;

    const [items, total] = await Promise.all([
      Service.find(query)
        .select('name description mainCategory subCategory price min max imageUrl plans qualityScore')
        .sort(dbSort)
        .skip(skip)
        .limit(limit)
        .lean(),
...
        // حساب السعر النهائي للكمية الأدنى والأقصى
        priceForMinQuantity: Number((Number(s.price) * (s.min / 1000) * (1 + PROFIT_MARGIN)).toFixed(4)),
        priceForMaxQuantity: Number((Number(s.price) * (s.max / 1000) * (1 + PROFIT_MARGIN)).toFixed(4))
      };
    });
    
    // الفرز في الذاكرة بعد حساب السعر لكل وحدة
    if (sortBy === 'pricePerQuantity') {
      servicesWithCalculations.sort((a, b) => {
        if (sortDir === -1) {
          return b.pricePerUnit - a.pricePerUnit; // تنازلي
        } else {
          return a.pricePerUnit - b.pricePerUnit; // تصاعدي
        }
      });
    }
...
      limit,
      items: servicesWithCalculations
    });
...
  res.json({
    ...
    pricePerUnit: pricePerUnit,
    // حساب السعر النهائي للكمية الأدنى والأقصى
    priceForMinQuantity: Number((Number(service.price) * (service.min / 1000) * (1 + PROFIT_MARGIN)).toFixed(4)),
    priceForMaxQuantity: Number((Number(service.price) * (service.max / 1000) * (1 + PROFIT_MARGIN)).toFixed(4))
  });
});

// ---------------------------------------------
// دوال الإدارة والباقي كما هي
// ---------------------------------------------
