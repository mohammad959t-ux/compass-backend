...
    
    // ✅ تحسين: استخدم الفرز على مستوى قاعدة البيانات
    const sort = {};
...
      // ✅ إضافة منطق الترجمة هنا
      let translatedName = rawName;
      let translatedDesc = rawDesc || 'لا يوجد وصف';
      
      if (ENABLE_TRANSLATION) {
        try {
          const nameResult = await translate(rawName, { to: 'ar' });
          translatedName = nameResult.text;
          const descResult = await translate(rawDesc, { to: 'ar' });
          translatedDesc = descResult.text;
        } catch (translateErr) {
          console.error(`Failed to translate service ${apiServiceId}:`, translateErr);
          // في حالة فشل الترجمة، استمر باستخدام الأسماء الإنجليزية
        }
      }
...
      const basePriceForMin = dbPrice * (min / 1000);
      const finalPriceForMin = Number((basePriceForMin * (1 + PROFIT_MARGIN)).toFixed(4));
      
...
            $set: {
              price: dbPrice,
              name: translatedName, // ✅ استخدام الاسم المترجم
              description: translatedDesc, // ✅ استخدام الوصف المترجم
              mainCategory,
              subCategory,
              min,
              max,
...
// ---------------------------------------------
// دوال الإدارة الأخرى (بدون تغيير)
// ---------------------------------------------
...
    const [items, total] = await Promise.all([
      Service.find(query)
        .select('name description mainCategory subCategory price min max imageUrl isVisible pricePerUnit')
        .sort(sort)
        .skip(skip)
        .limit(limit)
        .lean(),
