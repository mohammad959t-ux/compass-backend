...
                    });
                    
                    // التحقق من وجود حقل "min" وتطبيق الحساب الصحيح
                    if (apiService.rate && apiService.min) {
                        // حساب سعر الوحدة الواحدة
                        const unitPrice = apiService.rate / apiService.min;
                        // تطبيق هامش الربح على سعر الوحدة ثم ضربه في الكمية الأدنى
                        newService.price = unitPrice * 1.2 * apiService.min;
                        newService.costPrice = apiService.rate;
                        newService.minQuantity = apiService.min;
                    } else if (apiService.rate) {
                        // إذا لم يكن هناك حقل "min" نستخدم السعر مباشرة
                        newService.price = apiService.rate * 1.2;
                        newService.costPrice = apiService.rate;
                        newService.minQuantity = 1;
                    }
